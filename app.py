...
def update_inventory_and_log():
    ...
    try:
        ...
        c.execute('''
            SELECT p.id, p.name, p.qty, p.product_qr_code_path, 
                   COUNT(DISTINCT al.id) as total_transactions,
                   SUM(CASE WHEN al.action_type = 'xuat_kho' THEN al.quantity ELSE 0 END) as total_exported
            FROM products p
            LEFT JOIN activity_log al ON p.id = al.product_id
            WHERE p.product_id_internal = ?
            GROUP BY p.id
        ''', (product_id,))
        product = c.fetchone()
        ...
        action_details = {
            ...
            'new_qty': new_qty,
            'total_transactions': product['total_transactions'] + 1,
            'total_exported': product['total_exported'] + (quantity if action == 'xuat' else 0)
        }

        c.execute('''
            INSERT INTO activity_log (
                user_email, action_type, product_id, product_name, 
                quantity, action_date, details
            )
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            session['username'],
    ...
    try:
        ...
        c.execute('''
            SELECT 
                ...
                SUM(p.qty) as total_inventory,
                COUNT(DISTINCT al.id) as total_transactions,
                COUNT(DISTINCT CASE WHEN p.qty <= 5 THEN p.id END) as low_stock_count,
                COUNT(DISTINCT CASE WHEN p.qty = 0 THEN p.id END) as out_of_stock_count
            FROM products p
            LEFT JOIN activity_log al ON p.id = al.product_id
        ''')
        overview_stats = c.fetchone()
        
        # Lấy 5 sản phẩm tồn kho thấp
        c.execute('''
            SELECT name, qty 
            FROM products 
            ...
            LIMIT 5
        ''')
        low_stock_products = [dict(row) for row in c.fetchall()]
        ...
        c.execute('''
            SELECT 
                ...
                SUM(CASE WHEN action_type = 'xuat_kho' THEN quantity ELSE 0 END) as total_exported,
                SUM(CASE WHEN action_type = 'nhap_kho' THEN quantity ELSE 0 END) as total_imported
            FROM activity_log
